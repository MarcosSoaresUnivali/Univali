...
if(opc == 1):
    ...
    plt.stem(dim_h)  # normaliza o filtro
    plt.title('Passa Alta com Hamming Normalizado')
    plt.show()

    [w, H] = sf.freqz(dim_h, fs=Fs)  # freqz normalizado
    freq = w
    plt.plot(freq, abs(H))
    plt.title('Magnitude da resposta em frequencia, Passa Alta Hamming normalizado')
    plt.show()

    res_conv = np.convolve(dim_h, data)  # conv com sweep normalizado

    plt.plot(res_conv)
    plt.title('Conv. com Sweep, Passa Alta com Hamming Normalizado')
    plt.show()
    ...
    plt.stem(dim_h)  # normaliza o filtro
    plt.title('Passa Alta com Blackman Normalizado')
    plt.show()

    [w, H] = sf.freqz(dim_h, fs=Fs)  # freqz normalizado
    freq = w
    plt.plot(freq, abs(H))
    plt.title('Magnitude da resposta em frequencia, Passa Alta Blackman normalizado')
    plt.show()

    res_conv = np.convolve(dim_h, data)  # conv com sweep normalizado

    plt.plot(res_conv)
    plt.title('Conv. com Sweep, Passa Alta com Blackman Normalizado')
    plt.show()

with open("Coeficiente_PA.dat", "w") as f:
    for s in dim_h:
        f.write(str(s) +",\n")
